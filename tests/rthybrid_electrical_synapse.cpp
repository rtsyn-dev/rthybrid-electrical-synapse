...
{
    if (!context || !value || port_index >= 5) return RTSYN_ABI_STATUS_INVALID_ARGUMENT;
...
TEST(RTHYBRID_ELECTRICAL_SYNAPSE, DescriptorShape)
{
    const auto *descriptor = rtsyn_rthybrid_electrical_synapse_get_descriptor();
    ASSERT_NE(descriptor, nullptr);
    EXPECT_STREQ(descriptor->name, "rthybrid_electrical_synapse");
    EXPECT_EQ(descriptor->node_type, RTSYN_ABI_NODE_PLUGIN);
    EXPECT_EQ(descriptor->port_count, 5U);
    EXPECT_EQ(descriptor->param_count, 1U);
    EXPECT_EQ(descriptor->state_count, 6U);
}

TEST(RTHYBRID_ELECTRICAL_SYNAPSE, ProcessesAndPublishesFiniteOutput)
{
    const auto *descriptor = rtsyn_rthybrid_electrical_synapse_get_descriptor();
...
TEST(RTHYBRID_ELECTRICAL_SYNAPSE, StateAndInvalidAccess)
{
    const auto *descriptor = rtsyn_rthybrid_electrical_synapse_get_descriptor();
    void *instance = nullptr;
    double value = 1.0;
